...
/**
 * POST /api/files/:id/thumbnail - Upload encrypted thumbnail to Discord
 * Body: multipart/form-data with 'thumbnail' file, iv, authTag
 */
router.post('/:id/thumbnail', memoryUpload.single('thumbnail'), asyncHandler(async (req, res) => {
  const fileId = parseInt(req.params.id, 10);
  const file = db.getFileById(fileId);
  ...
  if (!iv || !authTag) {
    throw new ApiError(400, 'iv and authTag are required');
  }

  // Upload thumbnail to Discord
  const result = await discord.sendFile(req.file.buffer, `thumb_${file.original_name}.enc`);

  // Store thumbnail metadata in database
  db.updateFileThumbnail(fileId, {
...
/**
 * GET /api/files/:id/thumbnail - Get thumbnail data (URL + crypto info)
 */
router.get('/:id/thumbnail', asyncHandler(async (req, res) => {
  ...
  const thumbnail = db.getFileThumbnail(fileId);
  ...
  res.json({
    success: true,
    thumbnail: {
      url: thumbnail.discordUrl,
      iv: thumbnail.iv,
      authTag: thumbnail.authTag,
      size: thumbnail.size,
    },
  });
}));
...
/**
 * DELETE /api/files/:id/thumbnail - Delete thumbnail from Discord
 */
router.delete('/:id/thumbnail', asyncHandler(async (req, res) => {
  const fileId = parseInt(req.params.id, 10);
  const file = db.getFileById(fileId);
  ...
  if (thumbnail && thumbnail.messageId) {
    // Try to delete from Discord (best effort)
    await discord.deleteMessagesBulk([thumbnail.messageId]).catch(() => {});
  }

  db.clearFileThumbnail(fileId);
